...
#[test]
pub fn test_signature_help_tagged_templates_with_overloaded_tags8() {
    let mut t = TestingT;
    run_test_signature_help_tagged_templates_with_overloaded_tags8(&mut t);
}

fn run_test_signature_help_tagged_templates_with_overloaded_tags8(t: &mut TestingT) {
    if should_skip_if_failing("TestSignatureHelpTaggedTemplatesWithOverloadedTags8") {
        return;
    }
...
f `${ undefined }   ${ undefined }    ${/*1*/   10/*2*/./*3*/01 /*4*/} `"#;
    ...
    parameter_name: Some("p3_o2".to_string()),
    parameter_span: Some("p3_o2: number".to_string()),
