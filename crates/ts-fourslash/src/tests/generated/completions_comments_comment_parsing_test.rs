...
#[test]
pub fn test_completions_comments_comment_parsing() {
    let mut t = TestingT;
    run_test_completions_comments_comment_parsing(&mut t);
}

fn run_test_completions_comments_comment_parsing(t: &mut TestingT) {
    if should_skip_if_failing("TestCompletionsCommentsCommentParsing") {
        return;
    }
