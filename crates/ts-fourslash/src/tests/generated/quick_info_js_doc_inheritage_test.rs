...
#[test]
pub fn test_quick_info_js_doc_inheritage() {
    let mut t = TestingT;
    run_test_quick_info_js_doc_inheritage(&mut t);
}

fn run_test_quick_info_js_doc_inheritage(t: &mut TestingT) {
    if should_skip_if_failing("TestQuickInfoJsDocInheritage") {
        return;
    }
