#![allow(non_snake_case)]
#![allow(unused_imports)]

use crate::generated_prelude::*;
use ts_core as core;
use ts_ls as lsutil;
use ts_lsproto as lsproto;
use ts_modulespecifiers as modulespecifiers;

#[test]
pub fn test_js_doc_property_description5() {
    let mut t = TestingT;
    run_test_js_doc_property_description5(&mut t);
}

fn run_test_js_doc_property_description5(t: &mut TestingT) {
    if should_skip_if_failing("TestJsDocPropertyDescription5") {
        return;
    }
    let content = r"interface Multiple1Example {
    /** Something generic */
    [key: number | symbol | `data-${string}` | `data-${number}`]: string;
}
function multiple1Example(e: Multiple1Example) {
    console.log(e./*multiple1*/anything);
}";
    let (mut f, done) = new_fourslash(t, None /*capabilities*/, content.to_string());
    f.verify_quick_info_at(t, "multiple1", "any", "");
    done();
}
