...
#[test]
pub fn test_signature_help_tagged_templates_with_overloaded_tags9() {
    let mut t = TestingT;
    run_test_signature_help_tagged_templates_with_overloaded_tags9(&mut t);
}

fn run_test_signature_help_tagged_templates_with_overloaded_tags9(t: &mut TestingT) {
    if should_skip_if_failing("TestSignatureHelpTaggedTemplatesWithOverloadedTags9") {
        return;
    }
...
f `${/*1*/ /*2*/ /*3*/}   ${"#;
    ...
    parameter_name: Some("p1_o2".to_string()),
    parameter_span: Some("p1_o2: number".to_string()),
