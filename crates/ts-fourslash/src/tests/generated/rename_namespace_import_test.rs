...
#[test]
pub fn test_rename_namespace_import() {
    let mut t = TestingT;
    run_test_rename_namespace_import(&mut t);
}

fn run_test_rename_namespace_import(t: &mut TestingT) {
    if should_skip_if_failing("TestRenameNamespaceImport") {
        return;
    }
