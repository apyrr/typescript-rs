...
#[test]
pub fn test_references_in_string_literal_value_with_multiple_projects() {
    let mut t = TestingT;
    run_test_references_in_string_literal_value_with_multiple_projects(&mut t);
}

fn run_test_references_in_string_literal_value_with_multiple_projects(t: &mut TestingT) {
    if should_skip_if_failing("TestReferencesInStringLiteralValueWithMultipleProjects") {
        return;
    }
