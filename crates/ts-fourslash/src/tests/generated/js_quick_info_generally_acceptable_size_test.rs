...
#[test]
pub fn test_js_quick_info_generally_acceptable_size() {
    let mut t = TestingT;
    run_test_js_quick_info_generally_acceptable_size(&mut t);
}

fn run_test_js_quick_info_generally_acceptable_size(t: &mut TestingT) {
    if should_skip_if_failing("TestJsQuickInfoGenerallyAcceptableSize") {
        return;
    }
...
var anotherThing/*2*/ = function(a, b) {}";
    ...
    f.verify_quick_info_at(t, "2", "var anotherThing: SomeCallback", "Another thing");
    done();
}
