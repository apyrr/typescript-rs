...
#[test]
pub fn test_completions_js_doc_tags() {
    let mut t = TestingT;
    run_test_completions_js_doc_tags(&mut t);
}

fn run_test_completions_js_doc_tags(t: &mut TestingT) {
    if should_skip_if_failing("TestCompletionsJSDocTags") {
        return;
    }
    let content = r"/**
 * This is class Foo.
