...
#[test]
pub fn test_completions_import_promote_type_only7() {
    let mut t = TestingT;
    run_test_completions_import_promote_type_only7(&mut t);
}

fn run_test_completions_import_promote_type_only7(t: &mut TestingT) {
    if should_skip_if_failing("TestCompletionsImport_promoteTypeOnly7") {
        return;
    }
...
new SomePig/**/"#;
    ...
    f.verify_apply_code_action_from_completion(
        ...
        &ApplyCodeActionFromCompletionOptions {
            ...
            description: "Remove 'type' from import declaration from \"./exports.ts\"".to_string(),
            new_file_content: Some(
                r#"import { SomePig } from "./exports.ts";
new SomePig"#
                    .to_string(),
            ),
