...
#[test]
pub fn test_signature_help_comments_class_members() {
    let mut t = TestingT;
    run_test_signature_help_comments_class_members(&mut t);
}

fn run_test_signature_help_comments_class_members(t: &mut TestingT) {
    if should_skip_if_failing("TestSignatureHelpCommentsClassMembers") {
        return;
    }
