...
#[test]
pub fn test_signature_help_call_expression() {
    let mut t = TestingT;
    run_test_signature_help_call_expression(&mut t);
}

fn run_test_signature_help_call_expression(t: &mut TestingT) {
    if should_skip_if_failing("TestSignatureHelpCallExpression") {
        return;
    }
