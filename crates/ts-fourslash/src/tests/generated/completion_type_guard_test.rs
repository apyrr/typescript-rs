...
#[test]
pub fn test_completion_type_guard() {
    let mut t = TestingT;
    run_test_completion_type_guard(&mut t);
}

fn run_test_completion_type_guard(t: &mut TestingT) {
    if should_skip_if_failing("TestCompletionTypeGuard") {
        return;
    }
