...
#[test]
pub fn test_rename_destructuring_assignment_nested_in_for_of() {
    let mut t = TestingT;
    run_test_rename_destructuring_assignment_nested_in_for_of(&mut t);
}

fn run_test_rename_destructuring_assignment_nested_in_for_of(t: &mut TestingT) {
    if should_skip_if_failing("TestRenameDestructuringAssignmentNestedInForOf") {
        return;
    }
