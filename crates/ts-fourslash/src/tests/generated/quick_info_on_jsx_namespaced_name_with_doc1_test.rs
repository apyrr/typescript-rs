#![allow(non_snake_case)]
#![allow(unused_imports)]

use crate::generated_prelude::*;
use ts_core as core;
use ts_ls as lsutil;
use ts_lsproto as lsproto;
use ts_modulespecifiers as modulespecifiers;

#[test]
pub fn test_quick_info_on_jsx_namespaced_name_with_doc1() {
    let mut t = TestingT;
    run_test_quick_info_on_jsx_namespaced_name_with_doc1(&mut t);
}

fn run_test_quick_info_on_jsx_namespaced_name_with_doc1(t: &mut TestingT) {
    if should_skip_if_failing("TestQuickInfoOnJsxNamespacedNameWithDoc1") {
        return;
    }
    let content = r#"// @jsx: react
// @Filename: /types.d.ts
declare namespace JSX {
  interface IntrinsicElements {
    'my-el': {
      /** This appears */
      foo: string;

      /** This also appears */
      'prop:foo': string;
    };
  }
}
// @filename: /a.tsx
<my-el /*1*/prop:foo="bar" /*2*/foo="baz" />"#;
    let (mut f, done) = new_fourslash(t, None /*capabilities*/, content.to_string());
    f.verify_quick_info_at(t, "1", "(property) 'prop:foo': string", "This also appears");
    f.verify_quick_info_at(t, "2", "(property) foo: string", "This appears");
    done();
}
